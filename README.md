...
1. **Vue d'ensemble** (`/`) : 4 KPI macro (prix médian au m², volume, valeur échangée, indice de tension), bande de dispersion P10-P90, volume mensuel, base 100 par département, communes en mouvement.
2. **Explorateur & Analytics** (`/explorer`) : filtres multi-critères, distribution du prix au m², nuage prix/surface avec droite d'élasticité, matrice de corrélation, structure du marché, classement des communes.
3. **Prédictions IA & Tendances** (`/predictions`) : simulateur d'estimation hédonique avec comparables et intervalle de confiance, prévision 12 mois (Holt), phases de marché par département, anomalies détectées (z-score robuste).
...
src/
  ...
  shared/
    api/          client Supabase, repository (bascule Supabase / mock), useQuery
    charts/       thème Recharts, ChartTooltip, Sparkline
    mocks/        jeux de données déterministes (PRNG à graine)
    types/        types du domaine, partagés par les mocks et les vues SQL
...
supabase/
  migrations/     schéma partitionné, table nettoyée, vues matérialisées, RLS, RPC
...
```
dvf_mutations (brut, partitionné par année)  <- n8n via Edge Function ingest-dvf
  | refresh_clean_mutations(from, to) : ventes simples, appart/maison, 200 à 30 000 €/m²
dvf_mutations_clean (1 ligne = 1 bien, price_per_sqm généré)
  | refresh_materialized_views()
mv_monthly_stats (mois × département × type : médiane, P10, P90, volume)
mv_commune_stats (année glissante par commune : médiane, variation N-1)
pipeline_runs / webhook_events               <- monitoring
```

Sécurité : RLS activée partout, lecture publique uniquement sur les agrégats et le monitoring, table brute invisible à la clé anon, écritures réservées à la `service_role` via Edge Functions authentifiées (HMAC ou secret partagé, comparaison en temps constant).
...
| Quels territoires divergent ? | Base 100 par département | LineChart multi-séries |
